<template>
  ...
  const options = ref(['общеобразовательный', 'Магистратура','—']);
  const selectedOptions = ref(JSON.parse(localStorage.getItem('selectedOptions')) || []);
  ...
  const displayText = computed(() => {
    if (selectedOptions.value.length === 0) return "Все виды";
    return selectedOptions.value.length === options.value.length ? "Все виды" : selectedOptions.value.join(", ");
  });
